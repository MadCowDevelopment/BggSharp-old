...
namespace BggSharp.Core
{
    public class BggPostClient
    {
        ...
        public async Task Login(string username, string password)
        {
            ...
            await using (Stream webpageStream = await webRequest.GetRequestStreamAsync())
            {
        ...
        {
            HttpClient httpClient = new(new HttpClientHandler { CookieContainer = _cookie });
        ...
        public async Task<Uri> PostGeeklistItem(string comments, int listId, int objectId, string objectType,
            string geekItemName, int imageId)
        {
            HttpClient httpClient = new(new HttpClientHandler { CookieContainer = _cookie });
            MultipartFormDataContent form = new();
            form.Add(new StringContent("save"), "action");
        ...
        public async Task<int> PostImage(string path, string filename)
        {
            ...
            var fs = File.OpenRead(Path.Combine(path, filename));
        ...
        public async Task<bool> LogPlay(int gameId, DateTime date, int amount, string comments, int length)
        {
            string requestBase =
